...
public class BbService {
	...
	@Autowired
	private BbExtend bbExtend;

	@PostMapping("/list")
	public String listBbApi(@RequestBody String sReq) {
		System.out.println("-------listBbApi:" + sReq);
		...
		try {
			...
			JSONArray jsaBMs = bbExtend.listBb(kd_id, doituong_kd);
			jout.put("list_bb", jsaBMs);
			jout.put("code", 200);
	...
	@PostMapping("/edit")
	public String editBbApi(@RequestBody String sReq) {
		System.out.println("-------editBbApi:" + sReq);
		JSONObject jout = new JSONObject();
		try {
			JSONObject jin = new JSONObject(sReq);
			String session_id = jin.getString("session_id");
			...
			int bm_id = jin.getInt("id");

			bbExtend.updateBb(so_vb, ngay_bh, bm_id, ghi_chu, sst.UserID);
	...
	@PostMapping("/delete")
	public String deleteBbApi(@RequestBody String sReq) {
		System.out.println("-------deleteBbApi:" + sReq);
		JSONObject jout = new JSONObject();
		try {
			JSONObject jin = new JSONObject(sReq);
			String session_id = jin.getString("session_id");
