...
public class BmExtend {
	...
	public JSONArray listBm(int kd_id, String doituong_kd) {
		JSONArray jsa = new JSONArray();
		String sql = "SELECT a.*, c.Fullname as Creator FROM TBL_Bieumau a "
	...
	public int updateBm(String so_vb, String ngay_bh, int bm_id, String ghi_chu, int created_by) {
		String sql = "update TBL_Bieumau set so_vb = ?, ngay_bh = CONVERT(DATETIME, ?, 102), ghi_chu = ?, CreatedTime = GETDATE(), CreatedBy = ? where ID = ?";
		return jdbcTemplate.update(sql, so_vb, ngay_bh, ghi_chu, created_by, bm_id);
	}

	public int deleteBm(int id) {
		return jdbcTemplate.update("update TBL_Bieumau set IsDeleted = 1 where ID = ?", id);
	}
}
